...
void print_help(const std::string& program_name, int max_line_length) {
    ...
    print_message(std::cout, "The BASE64 program is a command-line utility for encoding and decoding data in Base64 format. It supports various parameters and keys for configuring the encoding and decoding process, as well as formatting the output.", max_line_length);
    print_separator_line(std::cout, max_line_length);

    print_message(std::cout, "Usage:", max_line_length);
    print_message(std::cout, program_name + " [-e|-encode|-d|-decode] [-s|-separator_separator] [-header_header] [-footer_footer] [-p|-padding] [-text_text|-f|-file_file|-o|-output_output|-c|-columns_columns|-i|-input] [-h|-help]", max_line_length);
    print_separator_line(std::cout, max_line_length);

    print_message(std::cout, "Options:", max_line_length);
    print_message(std::cout, "  -e, -encode^^Encode input data to Base64 format (default).", max_line_length);
    print_message(std::cout, "  -d, -decode^^Decode Base64 input data to binary format.", max_line_length);
    ...
    print_message(std::cout, program_name + " -e -p -header_'-----BEGIN CERTIFICATE-----' -footer_'-----END CERTIFICATE-----' -f_certificate.crt -o_encoded-certificate.crt", max_line_length);
    print_message(std::cout, program_name + " -d -f_encoded-certificate.crt -o_decoded-certificate.crt", max_line_length);
    print_separator_line(std::cout, max_line_length);
}
...
// Function to encode input data to Base64 format
void encode(std::istream& input, std::ostream& output, const Parameters& params) {
    ...
    for (size_t i = 0; i < encoded.size(); ++i) {
        ...
        // Output the byte in Base64 format with the specified separator
        output << encoded[i];
...
// Function to decode Base64 input data to binary format
void decode(std::istream& input, std::ostream& output, const Parameters& params) {
    ...
    while (input.get(ch)) {
        ...
        if (base64_reverse_table[static_cast<unsigned char>(ch)] == -1) {
            print_message(std::cerr, "Invalid character: " + std::string(1, ch), params.max_chars);
            exit(1);
        }

        val = (val << 6) + base64_reverse_table[static_cast<unsigned char>(ch)];
        valb += 6;
        if (valb >= 0) {
